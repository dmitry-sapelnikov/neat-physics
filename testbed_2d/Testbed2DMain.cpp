...
int main()
{
	...
	auto worldCreator =
		[]()
		{
			auto result = std::make_unique<nph::World<2>>(
				nph::Vec2(0.0f, -nph::GRAVITY));
			result->reserveBodies(nph::BODIES_TO_RESERVE);
			return result;
		};

	nph::Testbed<2> testbed(worldCreator);

	auto sceneCreationCallback =
		[&glassSize, &sceneControl](nph::World<2>& world)
		{
			nph::createGlass(
				world,
				glassSize,
	...
	auto visualizationSetupCallback =
		[&glassSize](nph::Visualization& visualization)
		{
			visualization.setCameraPosition({
				0.0f,
				glassSize.y * 0.5f,
				glassSize.y * 1.5f });

			visualization.setCameraTarget({
				0.0f,
				glassSize.y * 0.5f,
				0.0f });

			visualization.setDepthTest(false);
	...
	testbed.setLeftMouseClickCallback(mouseClickCallback);
	testbed.setRightMouseDownCallback(mouseClickCallback);

	auto guiCallback =
		[&sceneControl]()
		{
			nph::drawSceneControlTab(sceneControl);
			nph::drawHelpTab();
		};
