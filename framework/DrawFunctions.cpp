...
void drawBody<2>(
	...
	const Vec<2>& halfSize,
	bool isStatic)
{
	const Mat22& rot = transform.rotation.getMat();
	const Vec2& pos = transform.position;
	const Vec2& hs = halfSize;
	...
	glColor4f(1.0f, 1.0f, 0.9f, isStatic ? 0.3f : 0.15f);
...
void drawBody<3>(
	...
	const Vec<3>& halfSize,
	bool isStatic)
{
	const Mat33& rot = transform.rotation.getMat();
	const Vec3& pos = transform.position;
	const Vec3& hs = halfSize;
