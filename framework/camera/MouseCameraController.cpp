...
void MouseCameraController::setRotationSpeed(float speed)
{
	assert(speed >= 0.0f);
	mRotationSpeed = speed;
}

void MouseCameraController::setZoomSpeed(float speed)
{
	assert(speed > 0.0f);
	mZoomSpeed = speed;
}
	...
	if (mouseInput.leftMouseDown && mRotationSpeed > 0.0f)
	{
		...
		else
		{
			...
			const Vec3 localDir = setDistanceAzimuthInclination(
				Vec3(1.0f, toRadians(mYaw), toRadians(mPitch)));
...
void MouseCameraController::onCameraChange()
{
