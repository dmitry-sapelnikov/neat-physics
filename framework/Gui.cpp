...
void drawVisualizationTab(nph::WorldDrawSettings& drawSettings)
{
	if (ImGui::CollapsingHeader("Visualization"))
	{
		...
		ImGui::Checkbox(
			"Contacts Points",
			&drawSettings.contactPoints);

		ImGui::Checkbox(
			"Contact Areas",
			&drawSettings.contactAreas);

		ImGui::Checkbox(
			"Contact Frames",
			&drawSettings.contactFrames);

		ImGui::SliderFloat(
			"Contact Point Size",
			&drawSettings.contactPointSize,
			2.0f,
			10.0f);

		ImGui::SliderFloat(
			"Contact Frame Size",
			&drawSettings.contactFrameSize,
			0.01f,
			0.5f);
	...
	SimulationControl& simulationControl)
{
		...
		ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		simulationControl.doSimulationStep = ImGui::Button(
			"Do Step",
			buttonsSize);
		...
		ImGui::Checkbox(
			...
			&simulationControl.fpsIndependentPhysics);
	}
}
