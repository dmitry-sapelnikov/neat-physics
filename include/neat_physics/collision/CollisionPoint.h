...
struct CollisionPoint
{
	...
	static constexpr uint32_t MAX_POINTS = (D == 2) ? 2 : 4;
	...
	struct GeometryFeature
	{
		...
		{
			return geometry == other.geometry && face == other.face;
		}
	...
	using GeometryFeaturePair = std::array<GeometryFeature, D>;
	...
	/// - penetration >= 0
	CollisionPoint(
		const Vec<D>& inPosition,
		const Vec<D>& inNormal,
		float inPenetration,
		const GeometryFeaturePair& inFeaturePair,
		uint32_t inClipBoxIndex,
		const std::array<Vec<D>, 2>& inLocalPoints,
		const Vec<D>& inLocalContactNormal
		) noexcept :
